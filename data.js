// data.js (final + localStorage + highlight rating 1/2 + Realtime Supabase)
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function loadSavedState() {
    try { return JSON.parse(localStorage.getItem("savedState"))||{}; }
    catch(e){ return {}; }
  }
  function persistSavedState(){ localStorage.setItem("savedState", JSON.stringify(savedState)); }
  ...
  function checkSaveButtonVisibility(){
    if(!allData || allData.length===0){ if(saveBtn) saveBtn.style.display="none"; return; }
    const changed = allData.some(r=>Boolean(r._checked)!==Boolean(r._saved));
    ...
    data.forEach((row,index)=>{
      ...
      columns.forEach(col=>{
        ...
        if(highlightCols.includes(col) && (String(value).trim()==="1" || String(value).trim()==="2")){
          td.style.backgroundColor="red"; td.style.color="white"; td.style.fontWeight="bold";
        }
        tr.appendChild(td);
      });
  ...
  function updateSelectAllState(){
    ...
    else if(checkedCount===rows.length){ selectAllCheckbox.checked=true; selectAllCheckbox.indeterminate=false; }
  ...
  async function fetchData(startDate,endDate){
    ...
    if(startDate && endDate){ query=query.gte("tgl",startDate.toISOString()).lte("tgl",endDate.toISOString()); }
    else {
      const now=new Date();
    ...
    allData=(data||[]).map(r=>{
      const saved=savedState[r.id];
      r._saved = saved!==undefined?saved:r.is_saved===true;
      r._checked=r._saved;
      return r;
    });
...
if (saveBtn) {
  saveBtn.addEventListener("click", async () => {
    ...
    allData.forEach((row, index) => {
      ...
      const isChecked = cb ? cb.checked : Boolean(row._saved);
      row._checked = isChecked;
      row._saved = isChecked;
      row.is_saved = isChecked;
      savedState[row.id] = isChecked;
  ...
  if(selectAllCheckbox){
    selectAllCheckbox.addEventListener("change",(e)=>{
      ...
      allData.forEach((r,i)=>{
        ...
        if(checked) tr.classList.add("table-success"); else if(!r._saved) tr.classList.remove("table-success");
      });
      checkSaveButtonVisibility();
    });
  }
  ...
  supabase.channel('guest_comments_channel')
    .on('postgres_changes', { event: '*', schema: 'public', table: 'guest_comments' }, payload=>{
      ...
      if(rowIndex>=0){
        allData[rowIndex]._saved=updated.is_saved;
        allData[rowIndex]._checked=updated.is_saved;
        savedState[updated.id]=updated.is_saved;
        renderTable(allData);
      }
    })
    .subscribe();
